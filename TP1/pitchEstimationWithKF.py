...
if __name__=='__main__':
    ...
    for i in range(0, len(imuMes.time)):
        ...
        uk = computedPitchFromGyro[i] + imuMes.gyrY[i]*Te
        computedPitchFromGyro.append(uk)

        # prediction step of KF
        pitchKF.predict(uk)
    
        # compute pitch from acc measurements
        yk = np.arctan2(-imuMes.accX[i],np.sqrt(imuMes.accY[i]**2+imuMes.accZ[i]**2))
